import type { Post } from "../../../../hooks/useFetchPosts";

export default function PostDetails({ post }: { post: Post }) {
  return (
    <div className="post-details-wrapper flex flex-col items-center justify-center gap-12 max-w-[70rem]">
      <h1 className="section-header text-4xl font-semibold">Blog Details</h1>
      <div className="post-content flex flex-col items-start gap-12 justify-center w-full p-16 rounded-2xl transition-all duration-300 cursor-pointer hover:shadow-lg dark:bg-[#313131] bg-[#f1f3f5] dark:hover:shadow-md dark:hover:shadow-[#ec5e2a]">
        <h2 className="post-title text-[#ec5e2a] font-semibold text-3xl">
          {post.title}
        </h2>
        <p className="post-body text-[1.4rem]">{post.body}</p>

        <div className="post-stats flex gap-20">
          {/* Likes */}
          <div className="post-likes flex items-center  gap-4 text-[1.4rem]">
            <svg
              className="post-icon w-9 fill-[#ec5e2a]"
              xmlns="http://www.w3.org/2000/svg"
              width="32"
              height="32"
              fill="#000000"
              viewBox="0 0 256 256"
            >
              <path d="M234,80.12A24,24,0,0,0,216,72H160V56a40,40,0,0,0-40-40,8,8,0,0,0-7.16,4.42L75.06,96H32a16,16,0,0,0-16,16v88a16,16,0,0,0,16,16H204a24,24,0,0,0,23.82-21l12-96A24,24,0,0,0,234,80.12ZM32,112H72v88H32ZM223.94,97l-12,96a8,8,0,0,1-7.94,7H88V105.89l36.71-73.43A24,24,0,0,1,144,56V80a8,8,0,0,0,8,8h64a8,8,0,0,1,7.94,9Z"></path>
            </svg>
            <span className="post-likes-count font-semibold">{post.likes}</span>
          </div>

          {/* Dislikes */}
          <div className="post-dislikes flex items-center gap-4 text-[1.4rem]">
            <svg
              className="post-icon w-9 fill-[#ec5e2a]"
              xmlns="http://www.w3.org/2000/svg"
              width="32"
              height="32"
              fill="#000000"
              viewBox="0 0 256 256"
            >
              <path d="M239.82,157l-12-96A24,24,0,0,0,204,40H32A16,16,0,0,0,16,56v88a16,16,0,0,0,16,16H75.06l37.78,75.58A8,8,0,0,0,120,240a40,40,0,0,0,40-40V184h56a24,24,0,0,0,23.82-27ZM72,144H32V56H72Zm150,21.29a7.88,7.88,0,0,1-6,2.71H152a8,8,0,0,0-8,8v24a24,24,0,0,1-19.29,23.54L88,150.11V56H204a8,8,0,0,1,7.94,7l12,96A7.87,7.87,0,0,1,222,165.29Z"></path>
            </svg>
            <span className="post-likes-count font-semibold">
              {post.dislikes}
            </span>
          </div>

          {/* Views */}
          <div className="post-views flex items-center  gap-4 text-[1.4rem]">
            <svg
              className="post-icon w-9 fill-[#ec5e2a]"
              xmlns="http://www.w3.org/2000/svg"
              width="32"
              height="32"
              fill="#000000"
              viewBox="0 0 256 256"
            >
              <path d="M247.31,124.76c-.35-.79-8.82-19.58-27.65-38.41C194.57,61.26,162.88,48,128,48S61.43,61.26,36.34,86.35C17.51,105.18,9,124,8.69,124.76a8,8,0,0,0,0,6.5c.35.79,8.82,19.57,27.65,38.4C61.43,194.74,93.12,208,128,208s66.57-13.26,91.66-38.34c18.83-18.83,27.3-37.61,27.65-38.4A8,8,0,0,0,247.31,124.76ZM128,192c-30.78,0-57.67-11.19-79.93-33.25A133.47,133.47,0,0,1,25,128,133.33,133.33,0,0,1,48.07,97.25C70.33,75.19,97.22,64,128,64s57.67,11.19,79.93,33.25A133.46,133.46,0,0,1,231.05,128C223.84,141.46,192.43,192,128,192Zm0-112a48,48,0,1,0,48,48A48.05,48.05,0,0,0,128,80Zm0,80a32,32,0,1,1,32-32A32,32,0,0,1,128,160Z"></path>
            </svg>
            <span className="post-views-count font-semibold">{post.views}</span>
          </div>
        </div>
      </div>
    </div>
  );
}
